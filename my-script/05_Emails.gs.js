...
// -- sendSupervisorDigestEmail_ --------------------------------
// Batched digest sent to the supervisor listing all employee-submitted
// pending items (col I = ⏳, col J empty) across all client trackers.
// items: [{ clientName, employee, monthTab, trackerUrl, pendingRows: [{ fileName, fileUrl, dateStr }] }]

function sendSupervisorDigestEmail_(supervisorEmail, supervisorName, items) {
  if (!supervisorEmail || !items || items.length === 0) return;
  const totalFiles = items.reduce(function(sum, it) { return sum + it.pendingRows.length; }, 0);
  const subject    = 'ملفات بانتظار الإرسال للعملاء (' + items.length + ' عميل، ' + totalFiles + ' ملف) — ' + COMPANY_NAME;

  let cardsHtml = '';
  for (let i = 0; i < items.length; i++) {
    const item  = items[i];
    const count = item.pendingRows.length;
    let fileRows = '';
    for (let j = 0; j < item.pendingRows.length; j++) {
      const bg = j % 2 === 0 ? '#ffffff' : '#f5f7fa';
      fileRows += `
        <tr style="background:${bg};">
          <td style="padding:9px 12px;font-size:13px;color:#2d3748;word-break:break-word;">${item.pendingRows[j].fileName}</td>
          <td style="padding:9px 12px;font-size:12px;color:#718096;text-align:center;white-space:nowrap;">${item.pendingRows[j].dateStr}</td>
        </tr>`;
    }
    cardsHtml += `
      <div style="border:1px solid #e8edf2;border-right:4px solid #8b5cf6;border-radius:8px;margin-bottom:20px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,0.06);">
        <div style="background:#8b5cf6;padding:12px 16px;">
          <span style="color:#ffffff;font-size:15px;font-weight:700;">${item.clientName}</span>
          <span style="color:rgba(255,255,255,0.85);font-size:13px;margin-right:12px;">${item.employee} · ${count} ملف(ات)</span>
        </div>
        <div style="padding:12px 16px 16px;">
          <table style="width:100%;border-collapse:collapse;border:1px solid #e8edf2;border-radius:6px;overflow:hidden;margin-bottom:12px;">
            <tr style="background:#f5f7fa;">
              <th style="padding:8px 12px;text-align:right;font-size:12px;color:#718096;font-weight:700;">اسم الملف</th>
              <th style="padding:8px 12px;text-align:center;font-size:12px;color:#718096;font-weight:700;">تاريخ الرفع</th>
            </tr>
            ${fileRows}
          </table>
          <a href="${item.trackerUrl}" style="display:inline-block;background:#1a73e8;color:#ffffff;padding:10px 22px;border-radius:6px;text-decoration:none;font-size:13px;font-weight:700;">
            افتح جدول المتابعة (${item.monthTab}) ←
          </a>
        </div>
      </div>`;
  }
  ...
  const content = `
    ${emailHeader_('#0d6b6e', 'ملفات بانتظار إرسالها للعملاء', items.length + ' عملية جاهزة للإرسال')}
    <div style="background:#ffffff;padding:24px 20px;border:1px solid #e8edf2;border-top:none;">
      <p style="font-size:14px;color:#4a5568;margin:0 0 20px 0;line-height:1.8;">
        مرحباً <strong>${supervisorName}</strong>، الملفات التالية اعتمدها الموظفون وهي بانتظار إرسالها للعملاء عبر جدول المتابعة:
      </p>
      ${cardsHtml}
    </div>
    ${emailFooter_('ar')}`;

  GmailApp.sendEmail(supervisorEmail, subject, '', { htmlBody: emailWrapper_(content, 'ar'), charset: 'UTF-8' });
}

// -- sendReturnToEmpEmail_ ------------------------------------
// Sent by the supervisor when returning files to an employee for revision.
// files: [{ name, url }]
