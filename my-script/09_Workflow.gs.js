...
// ── Backfill from legacy trackers ─────────────────────────────

/**
 * One-time idempotent backfill.
 * Walks all client tracker spreadsheets (same as notifyPendingSupervisors)
 * and inserts any missing rows into the `workflow` sheet.
 * Key: (clientFolderId, fileUrl).
 */
function backfillWorkflowFromTrackers() {
  const mainSsId = PropertiesService.getScriptProperties().getProperty('MAIN_SS_ID');
  if (!mainSsId) {
    Logger.log('backfillWorkflowFromTrackers: MAIN_SS_ID not set — run setupTriggers() first');
    return;
  }
  ...
  for (let i = 1; i < custData.length; i++) {
    ...
    const clientEmail = custData[i][COL_EMAIL         - 1];
    const clientLang  = custData[i][COL_LANG          - 1] || 'ar';

    if (!folderId || !employee) continue;

    const empEmail = getEmployeeEmail(employee, mainSs) || '';
    ...
    while (yearFolders.hasNext()) {
      ...
      const year       = parseInt(yearFolder.getName()) || null;
      if (!year) continue;
      ...
      while (filesIter.hasNext()) {
        ...
        for (let s = 0; s < sheets.length; s++) {
          ...
          const monthName = sheet.getName(); // e.g. "04-April"

          for (let r = 1; r < sheetData.length; r++) {
            const row      = sheetData[r];
            const fileName = String(row[1] || '').trim();
            const fileUrl  = String(row[2] || '').trim();
            if (!fileName || !fileUrl) continue;

            // Idempotency check
            if (wfFindByFileUrl_(fileUrl)) {
              skippedCount++;
              continue;
            }

            const uploadDate = row[4];
            const day        = uploadDate instanceof Date ? uploadDate.getDate() : null;
            const finished   = row[6] === true;
            const note       = String(row[7] || '');
            const submitStat = String(row[8] || '');
            const sendStat   = String(row[9] || '');

            let status = 'new';
            if (sendStat.indexOf('✅') === 0)         status = 'approved_sent';
            else if (submitStat.indexOf('⏳') === 0)   status = 'submitted';
            else if (finished)                          status = 'in_progress';

            const id  = wfUuid_();
            const now = new Date();
            const sh  = wfSheet_();
            sh.appendRow([
              id, folderId, clientName, clientEmail, clientLang,
              employee, empEmail, supName, supEmail,
              year, monthName, day,
              fileName, fileUrl, uploadDate || now,
              finished, note, status,
              submitStat.indexOf('⏳') === 0 ? uploadDate || '' : '',
              sendStat.indexOf('✅')   === 0 ? uploadDate || '' : '',
              0, '', now
            ]);
            insertedCount++;
            Utilities.sleep(50); // avoid hitting sheet quota
          }
        }
      }
    }
  ...
  Logger.log('backfillWorkflowFromTrackers: inserted=' + insertedCount + ' skipped=' + skippedCount);
}
