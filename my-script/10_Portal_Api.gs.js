...
function doGet(e) {
  ...
  try {
    switch (action) {
      ...
      // Write actions also available via GET (avoids CORS preflight on static portals)
      case 'setFinished':
        return handleSetFinished_({ id: params.id, finished: params.finished === 'true' }, roleInfo);
      case 'setNote':
        return handleSetNote_({ id: params.id, note: params.note || '' }, roleInfo);
      case 'submitDay':
        return handleSubmitDay_({
          clientFolderId: params.clientFolderId,
          year: params.year, month: params.month, day: params.day
        }, roleInfo);
      case 'approveAndSend':
        return handleApproveAndSend_({ ids: JSON.parse(params.ids || '[]') }, roleInfo);
      case 'returnToEmp':
        return handleReturnToEmp_({ ids: JSON.parse(params.ids || '[]'), returnNote: params.returnNote || '' }, roleInfo);
      default:
...
function handleListPending_(roleInfo) {
  ...
  var rows = wfListBySupEmail_(roleInfo.email, 'submitted');
...
function handleSubmitDay_(body, roleInfo) {
  ...
  // Block re-submit
  var alreadySubmitted = rows.find(function(r) { return r.status === 'submitted'; });
  if (alreadySubmitted) return err_('already submitted', 409);
  ...
  return ok_({ submitted: eligibleRows.length });
}
